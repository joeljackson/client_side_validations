    ...
    def client_side_form_settings(object, options)
      if options[:validate]
        ...
        content_tag(:script) do
          "//<![CDATA[\nif(window.ClientSideValidations===undefined)window.ClientSideValidations={};window.ClientSideValidations.disabled_validators=#{ClientSideValidations::Config.disabled_validators.to_json};window.ClientSideValidations.number_format=#{number_format.to_json};if(window.ClientSideValidations.patterns===undefined)window.ClientSideValidations.patterns = {};if(window.ClientSideValidations.has_validated===undefined)window.ClientSideValidations.has_validated = [] ;window.ClientSideValidations.patterns.numericality=#{patterns[:numericality]};if(window.ClientSideValidations.remote_validators_prefix===undefined)window.ClientSideValidations.remote_validators_prefix='#{(ClientSideValidations::Config.root_path||"").sub(/\/+\Z/,'')}';if(window.ClientSideValidations.forms===undefined)window.ClientSideValidations.forms={};window.ClientSideValidations.forms['#{var_name}'] = #{builder.client_side_form_settings(options, self).merge(:validators => 'validator_hash').to_json};if($('##{var_name}').validate && _.indexOf(window.ClientSideValidations.has_validated, '#{var_name}') == -1){$('##{var_name}').validate();window.ClientSideValidations.has_validated.push('#{var_name}');}\n//]]>".html_safe
        end
      end
    end
